...
def main_loop(
    ...
    best_n_trials = method(max_epochs=epochs,
           ...
           dataset=dataset,
           get_best_n_trials=1,  # Get this amount of best trials
        #    load_study=True,
        #    load_study_name="optuna_2"
           )
    ...
    for trial in best_n_trials:
        ...
        if not np.isnan(test_labels).any():
            ...
            with (output_path / f"trial_{trial_id}_score.yaml").open("a+") as f:
        ...
        with (output_path / f"trial_{trial_id}_score.yaml").open("a+") as f:
            yaml.safe_dump({"total_execution_time": f"{elapsed_minutes} minutes and {elapsed_seconds:.2f} seconds"}, f)
...
if __name__ == "__main__":
    ...
    main_loop(
        ...
        seed=seed,
        epochs= 15,
        data_fraction=0.1, # "Subsampling of training set, in fraction (0, 1]. 1 is all the data"
        val_percentage = 0.2,
    )
